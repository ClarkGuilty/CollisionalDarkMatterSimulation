...
#    plt.imshow(image,extent=[constantes[0],constantes[3],constantes[0],constantes[3]],norm=matplotlib.colors.LogNorm(vmin = clim[0], vmax = clim[1]), interpolation='nearest', aspect='auto')
    plt.imshow(image,extent=[constantes[0],constantes[3],constantes[0],constantes[3]],norm=matplotlib.colors.PowerNorm(0.4,vmin = 0, vmax = 4.5),interpolation='nearest', aspect='auto')
    cbar = plt.colorbar()
    if(clim != None):
