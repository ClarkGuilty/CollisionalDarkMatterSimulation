...
void printPhaseX(char *name, int corteY, int corteVy, int corteZ ,int corteVz)
{
	...
	for(i=0;i<Nx;i+=1) {
        ...
        for(j=0;j<Nvx;j+=1){             
            ...
            fprintf(output,"%f ",convertir(phase[ind(i,corteY,corteZ,j,corteVy,corteVz)], aMasasSol)/pow(convertir(1.0,aKpc)*(convertir(1.0,aKpc)*3.0857e+19),2)*pow(convertir(1.0,aSegundos),3));
        }
		fprintf(output,"\n");
			}
	fclose(output);
